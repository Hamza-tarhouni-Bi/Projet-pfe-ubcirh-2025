...
const sendUpdateDemandeAvanceEmail = async (email, nom, prenom, montant, type, statut, motif) => {
  try {
    ...
    if (statut === 'Approuvée') {
      statutColor = 'green';
      statutMessage = `
        <p>Le montant de <strong>${montant} TND</strong> sera versé selon les procédures en vigueur.</p>
    ...
    const mailOptions = {
      ...
      to: email,
      subject: `Mise à jour de votre demande d'avance (${type})`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 5px;">
          <div style="text-align: center; padding: 10px; background-color: #f8f8f8; margin-bottom: 20px;">
            <h2 style="color: #333;">Statut de votre demande d'avance</h2>
          </div>
          
          <p>Bonjour <strong>${prenom} ${nom}</strong>,</p>
          
          <p>Nous vous informons que votre demande d'avance de type <strong>${type}</strong> d'un montant de <strong>${montant} TND</strong> a été <strong style="color: ${statutColor}">${statut}</strong>.</p>
          
          <p>Détails de votre demande :</p>
          <ul>
            <li>Type d'avance: ${type}</li>
            <li>Montant demandé: ${montant} TND</li>
    ...
    console.log('Email de statut d\'avance envoyé: %s', info.messageId);
    return true;
  } catch (error) {
    console.error('Erreur lors de l\'envoi de l\'email de mise à jour d\'avance:', error);
    throw error;
  }
};

module.exports = {
  sendUpdateDemandeAvanceEmail
};